...
const CardContainer = () => {
  ...
  const setAnimationStartPosition = (startingCard) => {
    if (appState === appStates.HOME) {
      setStartPos(
        positionInContainer(startingCard, references.current.container)
      );
    }
  };
  ...
  const toHome = () => {
    ...
    tl.add(
      gsap.to(references.current.cardFull, {
        x: startPos.x,
        y: startPos.y,
        duration: 0.4,
        height: startPos.height - 6,
    ...
    tl.add(
      gsap.to(references.current.cardFullContents, {
        opacity: 0,
        duration: 0.1
      }),
      0
  ...
  return (
    <Container ref={(el) => (references.current.container = el)}>
      {appState === appStates.HOME && (
        <>
          <PomodoroCard ref={(el) => (references.current.card1 = el)}>
            <CardContents
              flex
              ref={(el) => (references.current.card1Content = el)}>
      ...
      {appState === appStates.COUNTDOWN && (
        <FullCard ref={(el) => (references.current.cardFull = el)}>
          <CardContents
            flex
            ref={(el) => (references.current.cardFullContents = el)}>
            <CountdownContents
              title={timer.name}
